...
def plots(opt):
	...
	plt.title('training cost with batch size='+opt)
...
def tnt(batch_size, hnno, decay, trainX, trainY):
	...
	times.append(1000*(time.time()-t1)/(epochs*n//batch_size))
	print('Time: %.2fs'%(time.time()-t1))
	print('%.2f accuracy at %d iterations'%(np.max(test_accuracy)*100, np.argmax(test_accuracy)+1))

def timeplot(opt):
	plt.figure()
	xtick = [0,1,2,3,4]
	plt.xticks(xtick,opt)
	plt.plot(xtick, times)
	plt.xlabel('Batch size')
	plt.ylabel('Time ms')
	plt.title('Update time with batch size')
	plt.savefig('p1a_batch_time.png')

times = []
batch_sizeA = [4,8,16,32,64]

for batch_size in batch_sizeA:
	hnno = 10
	decay = 1e-6
	test_accuracy = []
	train_cost = []
	print('Batch size = %d running'%batch_size)
	tnt(batch_size, hnno, decay, trainX, trainY)
	plots(batch_size)
timeplot(batch_sizeA)