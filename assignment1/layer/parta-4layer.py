...
def initialize(hnno, decay):
	...
	#weights and biases from input to 1st-hidden layer
	w1, b1 = init_weights(36, 10), init_bias(10) 
	#weights and biases from 1st-hidden to 2nd-hidden layer
	w2, b2 = init_weights(10, 10), init_bias(10) 
	#weights and biases from 2nd-hidden to output layer
	w3, b3 = init_weights(10, 6, logistic=False), init_bias(6) 
	...
	cost = T.mean(T.nnet.categorical_crossentropy(py, Y)) \
				...
				+ decay*(T.sum(T.sqr(w2)+T.sum(T.sqr(w3))))
	# decay*(T.sum(T.sqr(w1)+T.sum(T.sqr(w2)+T.sum(T.sqr(w3)))))
...
def plots(opt):
	...
	plt.title('training cost with layer='+opt)
	...
	plt.title('test accuracy with layer='+opt)
